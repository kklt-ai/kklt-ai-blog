    ...
    <>
      ...
      <div className="absolute right-[25px] top-6 z-[4] inline-flex h-[15.32px] items-center gap-2 max-[640px]:right-[19px] max-[640px]:top-[29px] max-[640px]:gap-0">
        <nav className="group relative hidden items-center max-[640px]:inline-flex" aria-label="Mobile navigation">
          ...
          <div className="invisible absolute right-0 top-full z-10 flex min-w-40 translate-y-1.5 flex-col rounded-xl border border-black/10 bg-white px-2 py-3 opacity-0 shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition duration-300 group-hover:visible group-hover:translate-y-0.5 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0.5 group-focus-within:opacity-100" role="menu">
            {copy.cards.map((card) => (
              <a className="block whitespace-nowrap rounded-lg px-3.5 py-2 font-[var(--font-misans)] text-[15px] font-normal text-black no-underline transition-colors hover:bg-black/5" href={card.href} key={card.id} role="menuitem">
                {card.title}
              </a>
            ))}
          </div>
